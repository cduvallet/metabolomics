...
#mtab_start
RAW_DATA		True
SEQENCE_FILE		test_sequence_file.csv
...
class SummaryParser():
    def __init__(self, summary_file):
        ...
        self.attribute_value_mtab = {'PROCESSED': "N/A"}
    ...
    def SummaryFileChecker(self):
        ...
        for line in all_lines:
            if line.split('\t')[0].rstrip('\n\r') == "#16S_start":
                ...
                checksum += 1
        if checksum != 2:
            print "No 16S lines found in summary file."
        ...
        for line in all_lines:
            ...
            if line.split('\t')[0] == "PRIMERS_FILE" and len(line.split('\t')[1]) > 0:
                checksum += 1
            if (line.split('\t')[0] == "RAW_FASTQ_FILE" or line.split('\t')[0] == "RAW_FASTA_FILE" or line.split('\t')[0] == "RAW_FASTQ_FILES") and len(line.split('\t')[1]) > 0:
                checksum += 1
        if checksum != 4:
    ...
    def ExtractMtabLines(self):
        # Description:  Extracts the line numbers pertaining to metabolomics
        with open(self.summary_file,'r') as summary_fid:
            all_lines = summary_fid.readlines()
            for i in range(len(all_lines)):
                line = all_lines[i].split()
                if(len(line)>0):
                    if(line[0] == "#mtab_start"):
                        startline = i+1
                    if(line[0] == "#mtab_end"):
                        endline = i
                        break
    ...
    def ReadSummaryFile(self):
        ...
        with open(self.summary_file,'r') as summary_fid:
            ...
            for i in range(startline, endline):
                ...
                attribute = line[0]
                value = line[1].rstrip()
                self.attribute_value_mtab[attribute] = value
    ...
    def WriteSummaryFile(self):
        ...
        with open(self.summary_file,'w') as summary_fid:
            ...
            for [key, value] in self.attribute_value_mtab.items():
                summary_fid.write(key + '\t' + value + '\n')
            summary_fid.write('#mtab_end\n')
